...
/* handle case of call where op0/op1 is "=mf" and opn is "mrf"
   eg. fadd */
#define FP_CALL(op, rev, n) fp_call_internal (op, rev, n, operands, insn);
  ...
  if (REG_P (target))
    {
      ...
      output_asm_insn (AS2 (add%L,%2,%0), xoperands);
      output_asm_insn ("fst%S %1", xoperands);
      output_asm_insn ("pop%L %0", &target);
    }
  else if (GET_CODE (target) == MEM)
    output_asm_insn ("fst%S %0", &target);
}
  ...
  if (REG_P (target))
    {
      ...
      xoperands[2] = gen_rtx (CONST_INT, VOIDmode, -4);
      output_asm_insn (AS2 (add%L,%2,%0), xoperands);
      output_asm_insn ("fstp%S %1", xoperands);
      output_asm_insn ("pop%L %0", &target);
      /* fp_pop_level--; */
    }
  else if (GET_CODE (target) == MEM)
    {
      /* fp_pop_level--; */
      output_asm_insn ("fstp%S %0", &target);
    }
  else abort ();
}

  ...
  if (REG_P (target))
    {
      ...
      output_asm_insn (AS2 (add%L,%3,%0), xoperands);
      output_asm_insn ("fst%Q %2", xoperands);
      output_asm_insn ("pop%L %0", &target);
      output_asm_insn ("pop%L %1", xoperands);
    }
  else if (GET_CODE (target) == MEM)
    output_asm_insn ("fst%Q %0", &target);
}
  ...
  if (REG_P (target))
    {
      ...
      output_asm_insn (AS2 (add%L,%3,%0), xoperands);
      /* fp_pop_level--; */
      output_asm_insn ("fstp%Q %2", xoperands);
      output_asm_insn ("pop%L %0", &target);
      output_asm_insn ("pop%L %1", xoperands);
    }
  else if (GET_CODE (target) == MEM)
  ...
  if (REG_P (target) || GET_MODE (target) != SImode)
    {
      ...
      output_asm_insn (AS2 (sub%L,%1,%0), xxops);
      xxops[0] = AT_SP (Pmode);
      /* fp_pop_level--; */
  ...
  if (REG_P (x))
    {
      ...
      output_asm_insn ("fld%S %0 \n\tadd%L %2,%1", xfops);
    }
  else
    output_asm_insn ("fld%S %0", &x);
}
  ...
  if (REG_P (x))
    {
      ...
      output_asm_insn ("push%L %1", xoperands);
      output_asm_insn ("push%L %0", xoperands);
      xfops[0] = AT_SP (Pmode);
      xfops[2] = gen_rtx (CONST_INT, VOIDmode, 8);
      xfops[1] = stack_pointer_rtx;
      output_asm_insn ("fld%Q %0 \n\tadd%L %2,%1", xfops);
    }
  ...
  if (n < 0)
    {
      ...
      output_asm_insn (AS2 (sub%L,%0,%1), xops);
    }
  else if (n > 0)
    {
      xops[0] = gen_rtx (CONST_INT, VOIDmode, n);
      output_asm_insn (AS2 (add%L,%0,%1), xops);
    }
}
  ...
  if (frame_pointer_needed)
    {
      ...
      output_asm_insn (AS2 (mov%L,%0,%1), xops);
      if (size)
	output_asm_insn (AS2 (sub%L,%2,%0), xops);
    }
     ...
     int size;
{
  ...
  for (regno = (limit -1); regno >= 2; regno--)
    if (regs_ever_live[regno] && ! dont_save_regs[regno])
      nregs++;
  ...
  if (nregs && frame_pointer_needed)
    {
      ...
      output_asm_insn (AS2 (lea%L,%0,%1), xops);
    }
  for (regno = (limit -1); regno >= 2; regno--)
    {
      if (regs_ever_live[regno] && ! dont_save_regs[regno])
